...
class TimelineService
{
    ...
    public function getFeed(int $perPage = 15)
    {
        $user = Auth::user();
        if (!$user->couple_id) {
            throw new Exception('User does not belong to a couple.');
        }

        return $this->repository->getCouplePosts($user->couple, $perPage);
    }
    ...
    public function toggleReaction(int $postId, string $emoji): array
    {
        ...
        $result = $this->repository->toggleReaction($post, $user, $emoji);
        $action = $result['action'];

        // Create notification for post author (if not the reactor and action is not removed)
        if ($post->user_id !== $user->id && $action !== 'removed') {
            $emojiMap = [
                'heart' => '❤️',
                'laugh' => '😂',
                'wow' => '😮',
            ...
            Notification::create([
                ...
                'post_id' => $post->id,
                'actor_id' => $user->id,
                'message' => "{$user->name} merespons postinganmu dengan {$emojiChar}",
                'link' => route('timeline.show', $post->id),
                'is_read' => false,
            ]);
        }
    ...
    public function addComment(int $postId, string $content): PostComment
    {
        ...
        if ($post->user_id !== $user->id) {
            $truncatedContent = strlen($content) > 50 ? substr($content, 0, 50) . '...' : $content;
            ...
            Notification::create([
                ...
                'post_id' => $post->id,
                'actor_id' => $user->id,
                'message' => "{$user->name} berkomentar: \"{$truncatedContent}\"",
                'link' => route('timeline.show', $post->id),
                'is_read' => false,
            ]);
        }
     ...
     */
    public function getComments(int $postId, int $perPage = 20)
    {
        $post = $this->findPost($postId);
        return $this->repository->getPostComments($post, $perPage);
    }
    ...
    {
        $maxSize = $postType === 'voice_note' ? 5 * 1024 * 1024 : 10 * 1024 * 1024; // 5MB audio, 10MB images
    ...
    protected function validateEmoji(string $emoji): void
    {
        ...
        if (!in_array($emoji, $validEmojis)) {
            throw new Exception('Invalid emoji type.');
        }
    }
    ...
    protected function verifyPostAccess(TimelinePost $post): void
    {
        ...
        if (!$user->couple_id || $post->couple_id !== $user->couple_id) {
            throw new Exception('You do not have access to this post.');
        }
    }
}
